...
#line 742
typedef int (*__compar_fn_t)(const void *arg_0x2b48b43f31f0, const void *arg_0x2b48b43f34c8);
#line 776
__extension__ 
#line 793
__extension__ 
...
unsigned int (*hashfunction)(void *arg_0x2b48b46c3868), 
int (*key_eq_fn)(void *arg_0x2b48b46c2020, void *arg_0x2b48b46c22c0));
#line 103
#line 102
int 
...
static inline void RandomInitialise(int arg_0x2b48b48b2288, int arg_0x2b48b48b24f0);
static inline double RandomUniform(void );
...
static void BFC$Boot$booted(void );
# 37 "SHA1.nc"
static int SHA1C$SHA1$reset(SHA1Context *context);
static int SHA1C$SHA1$update(SHA1Context *context, const uint8_t *message_array, uint32_t length);
static int SHA1C$SHA1$digest(SHA1Context *context, uint8_t Message_Digest[20]);
# 62 "/opt/tinyos-2.1.2/tos/interfaces/Init.nc"
...
# 37 "SHA1.nc"
static int BFC$SHA1$reset(SHA1Context *context);
static int BFC$SHA1$update(SHA1Context *context, const uint8_t *message_array, uint32_t length);
static int BFC$SHA1$digest(SHA1Context *context, uint8_t Message_Digest[20]);
# 20 "BFC.nc"
SHA1Context BFC$context[1000];

uint8_t BFC$BFilter[1000][256 / 8];
uint8_t BFC$input[1000][20];
uint8_t BFC$output[1000][20];
uint8_t BFC$testHash[1000][20];
static inline void BFC$init(void );




static void BFC$createHash(uint8_t *ip, uint8_t *op);





static inline void BFC$mapToBF(void );
#line 52
static inline uint8_t BFC$verifyBF(void );
#line 75
static inline void BFC$initBF(void );
...
static inline void BFC$Boot$booted(void );
# 56 "SHA1C.nc"
static inline void SHA1C$SHA1PadMessage(SHA1Context *arg_0x2b48b4ec18a8);
static void SHA1C$SHA1ProcessMessageBlock(SHA1Context *arg_0x2b48b4ec0228);
#line 74
static inline int SHA1C$SHA1$reset(SHA1Context *context);
#line 115
static inline int SHA1C$SHA1$digest(SHA1Context *context, uint8_t Message_Digest[20]);
#line 167
static inline int SHA1C$SHA1$update(SHA1Context *context, const uint8_t *message_array, uint32_t length);
#line 230
static void SHA1C$SHA1ProcessMessageBlock(SHA1Context *context);
#line 386
static inline void SHA1C$SHA1PadMessage(SHA1Context *context);
...
# 78 "/opt/tinyos-2.1.2/tos/interfaces/Receive.nc"
inline static message_t * TossimActiveMessageC$Snoop$receive(am_id_t arg_0x2b48b4cc4020, message_t * msg, void * payload, uint8_t len){
#line 78
  nx_struct message_t *__nesc_result;
#line 78

#line 78
    __nesc_result = TossimActiveMessageC$Snoop$default$receive(arg_0x2b48b4cc4020, msg, payload, len);
#line 78
...
# 78 "/opt/tinyos-2.1.2/tos/interfaces/Receive.nc"
inline static message_t * TossimActiveMessageC$Receive$receive(am_id_t arg_0x2b48b4cc5340, message_t * msg, void * payload, uint8_t len){
#line 78
  nx_struct message_t *__nesc_result;
#line 78

#line 78
    __nesc_result = TossimActiveMessageC$Receive$default$receive(arg_0x2b48b4cc5340, msg, payload, len);
#line 78
...
# 69 "/opt/tinyos-2.1.2/tos/lib/tossim/SimSchedulerBasicP.nc"
static inline int SimSchedulerBasicP$sim_config_task_latency(void )
#line 69
{
#line 69
...
# 110 "/opt/tinyos-2.1.2/tos/interfaces/AMSend.nc"
inline static void TossimActiveMessageC$AMSend$sendDone(am_id_t arg_0x2b48b4cc6220, message_t * msg, error_t error){
#line 110
    TossimActiveMessageC$AMSend$default$sendDone(arg_0x2b48b4cc6220, msg, error);
#line 110
}
#line 110
...
# 75 "/opt/tinyos-2.1.2/tos/interfaces/TaskBasic.nc"
inline static void SimSchedulerBasicP$TaskBasic$runTask(uint8_t arg_0x2b48b49d4ab0){
#line 75
  switch (arg_0x2b48b49d4ab0) {
#line 75
...
#line 75
    default:
#line 75
      SimSchedulerBasicP$TaskBasic$default$runTask(arg_0x2b48b49d4ab0);
#line 75
      break;
#line 75
    }
...
#line 59
  for (i = 0; i < 20; i++) 
    {
      byteNum = BFC$testHash[sim_node()][i] / 8;
      bitNum = BFC$testHash[sim_node()][i] % 8;
      testBit = 0x80;
      testBit = testBit >> bitNum;
      if (BFC$BFilter[sim_node()][byteNum] & testBit) {
        result = 1;
        }
      else {
          result = 0;
...
#line 43
  for (i = 0; i < 20; i++) 
    {
      byteNum = BFC$output[sim_node()][i] / 8;
      bitNum = BFC$output[sim_node()][i] % 8;
      mapBit = 0x80;
      mapBit = mapBit >> bitNum;
      BFC$BFilter[sim_node()][byteNum] = BFC$BFilter[sim_node()][byteNum] | mapBit;
    }
}
...
static inline void BFC$init(void )
{
  BFC$input[sim_node()][0] = 1;
  BFC$input[sim_node()][10] = 10;
}
...
#line 87
  BFC$initBF();
  BFC$init();
  BFC$createHash(BFC$input[sim_node()], BFC$output[sim_node()]);
  BFC$mapToBF();
  for (i = 0; i < 256 / 8; i++) 
    {
      sim_log_debug(148U, "BFC", "BF[%d] = %d\n", i, BFC$BFilter[sim_node()][i]);
    }
  BFC$input[sim_node()][2] = 45;
  BFC$createHash(BFC$input[sim_node()], BFC$testHash[sim_node()]);
  res = BFC$verifyBF();
  if (res == 1) {
    sim_log_debug(149U, "BFC", "Bloom Filter verification passed\n");
    }
#line 100
  if (res == 0) {
    sim_log_debug(150U, "BFC", "Bloom Filter verification failed\n");
    }
}
...
# 37 "SHA1.nc"
inline static int BFC$SHA1$reset(SHA1Context *context){
#line 37
  int __nesc_result;
#line 37

#line 37
  __nesc_result = SHA1C$SHA1$reset(context);
#line 37
  ...
  while (length-- && ! context->Corrupted) {
      ...
      *message_array & 0xFF;
      ...
      if (context->Message_Block_Index == 64) {
          SHA1C$SHA1ProcessMessageBlock(context);
        }
...
# 38 "SHA1.nc"
inline static int BFC$SHA1$update(SHA1Context *context, const uint8_t *message_array, uint32_t length){
#line 38
  int __nesc_result;
#line 38

#line 38
  __nesc_result = SHA1C$SHA1$update(context, message_array, length);
#line 38
...
# 39 "SHA1.nc"
inline static int BFC$SHA1$digest(SHA1Context *context, uint8_t Message_Digest[20]){
#line 39
  int __nesc_result;
#line 39

#line 39
  __nesc_result = SHA1C$SHA1$digest(context, Message_Digest);
#line 39
...
static void BFC$createHash(uint8_t *ip, uint8_t *op)
{
  BFC$SHA1$reset(&BFC$context[sim_node()]);
  BFC$SHA1$update(&BFC$context[sim_node()], ip, 20);
  BFC$SHA1$digest(&BFC$context[sim_node()], op);
}
                               ...
                               uintptr_t* addr, size_t* size)
{
  ...
  /* Module BFC */
  if (!strcmp(varname, "BFC$context"))
  {
    *addr = (uintptr_t)&BFC$context[__nesc_mote];
    *size = sizeof(BFC$context[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "BFC$BFilter"))
  {
    *addr = (uintptr_t)&BFC$BFilter[__nesc_mote];
    *size = sizeof(BFC$BFilter[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "BFC$input"))
  {
    *addr = (uintptr_t)&BFC$input[__nesc_mote];
    *size = sizeof(BFC$input[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "BFC$output"))
  {
    *addr = (uintptr_t)&BFC$output[__nesc_mote];
    *size = sizeof(BFC$output[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "BFC$testHash"))
  {
    *addr = (uintptr_t)&BFC$testHash[__nesc_mote];
    *size = sizeof(BFC$testHash[__nesc_mote]);
    return 0;
  }
...
static void __nesc_nido_initialise(int __nesc_mote)
{
  ...
  /* Module BFC */
  memset((void *)&BFC$context[__nesc_mote], 0, sizeof BFC$context[__nesc_mote]);
  memset((void *)&BFC$BFilter[__nesc_mote], 0, sizeof BFC$BFilter[__nesc_mote]);
  memset((void *)&BFC$input[__nesc_mote], 0, sizeof BFC$input[__nesc_mote]);
  memset((void *)&BFC$output[__nesc_mote], 0, sizeof BFC$output[__nesc_mote]);
  memset((void *)&BFC$testHash[__nesc_mote], 0, sizeof BFC$testHash[__nesc_mote]);
